...
object QobuzAudioProvider {
    ...
    private fun requestCustomStream(
        baseUrl: String,
        track: MatchedTrack,
        qualityCode: Int,
        durationMs: Long?,
        ...
        val url = "$baseUrl/api/download-music".toHttpUrlOrNull()
            ...
            ?.build()
            ?: return StreamAttempt(error = "Custom resolver request URL could not be built")
        ...
        return runCatching {
            client.newCall(request).execute().use { response ->
                ...
                if (!response.isSuccessful) {
                    return@use StreamAttempt(error = "Custom resolver HTTP ${response.code}: ${payload.take(160)}")
                }
                if (payload.isBlank()) {
                    return@use StreamAttempt(error = "Custom resolver returned an empty response")
                }
                val root = JSONObject(payload)
                if (!root.optBoolean("success", false)) {
                    ...
                    return@use StreamAttempt(
                        error = "Custom resolver rejected quality $qualityCode: ${apiError ?: "unknown error"}"
                    )
                }
                ...
                val streamUrl = data?.stringOrNull("url")
                    ...
                    ?: return@use StreamAttempt(error = "Custom resolver did not return a stream URL for quality $qualityCode")
            ...
            StreamAttempt(error = "Custom resolver request failed: ${error.message ?: error.javaClass.simpleName}")
        }
    }
    ...
    private fun JSONObject.toCandidateMetadata(): CandidateMetadata? {
        ...
        return CandidateMetadata(
            trackId = trackId,
            title = displayTitle,
            artists = collectArtistNames(this),
            album = album,
